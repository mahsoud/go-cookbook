...
* `node['gocd']['agent']['autoregister']['enabled']`      - (Windows only)Whether or not an autoregister file is generated for the agent. If `true` the agent will autoregister with the settings supplied with the attributes below. Defaults to `false`.
